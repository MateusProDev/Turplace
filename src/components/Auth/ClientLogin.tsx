...
export default function ClientLogin() {
  ...
  useEffect(() => {
    if (user) {
      console.log("ClientLogin.tsx: Usuário logado, verificando dados...", { user: user.email, userData });

      // Aguardar os dados carregarem
      const checkAndRedirect = () => {
        console.log("ClientLogin.tsx: Verificando userData", { userData, isAdmin: userData?.isAdmin });

        if (userData !== null) {
          // Verifica se há um destino salvo no state
          const from = location.state?.from?.pathname;
          console.log("ClientLogin.tsx: Redirecionando cliente", { isAdmin: userData?.isAdmin, from });

          // Cliente vai para /client ou para o destino salvo (se não for /admin ou /provider)
          const destination = from && from !== "/admin" && from !== "/provider" ? from : "/client";
          console.log("ClientLogin.tsx: Redirecionando cliente para:", destination);
          navigate(destination, { replace: true });
        } else {
          console.log("ClientLogin.tsx: userData ainda null, tentando novamente em 500ms");
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    console.log("ClientLogin.tsx: handleSubmit chamado", { isLogin, email });
    e.preventDefault();
    setError("");
    setLoading(true);
    try {
  ...
  const handleGoogleLogin = async () => {
    console.log("ClientLogin.tsx: handleGoogleLogin chamado");
    setError("");
    setLoading(true);
    try {
      console.log("ClientLogin.tsx: Iniciando signInWithPopup");
      const result = await signInWithPopup(auth, googleProvider);
      console.log("ClientLogin.tsx: signInWithPopup sucesso", result.user.email);

      if (result.user) {
        console.log("ClientLogin.tsx: Salvando dados do usuário no Firestore");
        await setDoc(
        ...
        console.log("ClientLogin.tsx: Dados salvos com sucesso");
      }
      navigate("/client");
    } catch (err: unknown) {
      console.error("ClientLogin.tsx: Erro no Google login", err);
    ...
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-blue-100 to-emerald-100">
      <div className="bg-white/90 shadow-xl rounded-2xl p-8 w-full max-w-md flex flex-col items-center gap-6">
        <div className="flex flex-col items-center gap-2">
          ...
          <p className="text-gray-500 text-center">Acesse sua conta de cliente para explorar e comprar no marketplace.</p>
        </div>
        <form onSubmit={handleSubmit} className="w-full flex flex-col gap-3">
          {!isLogin && (
            <input value={name} onChange={e => setName(e.target.value)} placeholder="Seu nome completo" className="w-full px-4 py-2 border rounded-lg bg-gray-50 focus:outline-none focus:ring-2 focus:ring-blue-200" required />
          )}
